...
local function insert_to_best_inventory(stack, entity)
  ...
  if not success and entity.type == "inserter" then        
    ...
    success = entity.held_stack.transfer_stack(stack)    
    ...
    if success then
      ...
      if entity.drop_target then
        success = insert_to_best_inventory(stack, entity.drop_target)
        -- if success then
      ...
      if not success then
        ...
        game.surfaces[entity.surface_index].spill_item_stack({
  ...
  if not success then
    ...
    for i = 0, entity.get_max_inventory_index() - 1 do 
      local inventory = entity.get_inventory(entity.get_max_inventory_index() - i)
      if inventory then            
        local inserted = inventory.insert(stack)
  ...
  -- Failure.
  if not success then
    game.print("Correga - Bug warning: Could not output all " .. stack.name .. " to any inventory in " .. entity.name .. ". Items spilled to ground.")        
  end
...
local function handle_script_trigger_event(event)
  if event.effect_id == "correga-result-power-core" then    
    local result = {name="correga-power-core", count=1}
